...
def MILP_base(gisele_folder,case_study,n_clusters,coe,voltage,line1):
    ...
    power = instance.P
    subs = instance.k
    if voltage_constraint: #work around to simplify the script
        voltage = instance.E
    else:
        voltage = instance.Psub
    PS = instance.PPS
...
def MILP_base_losses(gisele_folder,case_study,n_clusters,coe,voltage,resistance,reactance,Pmax,line_cost):
    ############ Create abstract model ###########
    model = AbstractModel()
    data = DataPortal()
    MILP_input_folder = gisele_folder + '/Case studies/' + case_study + '/Intermediate/Optimization/MILP_input'
    ...
    model.losses1 = Var(model.links,within=Binary)
    model.losses2 = Var(model.links, within=Binary)
    model.losses3 = Var(model.links, within=Binary)
    model.FinalLosses = Var(model.links,within=NonNegativeReals)
    #####################Define constraints###############################
    model.positive_p = Var(model.links_clusters, within=Binary)
    # Radiality constraint
    def Radiality_rule(model):
    ...
    def Calculate_losses1(model, i, j):
        return model.losses1[i,j] *0.01 - 0.8 <= model.P[i,j]
    def Calculate_losses11(model, i, j):
        return model.P[i,j]<=model.losses1[i,j]*1
    def Calculate_losses2(model, i, j):
        return model.losses2[i,j]*1 <= model.P[i,j]
    def Calculate_losses22(model, i, j):
        return model.P[i,j]<=model.losses2[i,j]*2
    def Calculate_losses3(model, i, j):
        return model.losses3[i,j]*2 <= model.P[i,j]
    def Calculate_losses33(model, i, j):
        return model.P[i,j]<=model.losses3[i,j]*4
    def bound_losses(model,i,j):
        return model.losses1[i,j] + model.losses2[i,j] + model.losses3[i,j] <= 1

    def final_losses(model,i,j):
        return model.FinalLosses[i,j] == model.losses2[i,j]*2 + model.losses3[i,j]*8

    # model.Calculate_losses1 = Constraint(model.links,rule=Calculate_losses1)
    # model.Calculate_losses11 = Constraint(model.links,rule=Calculate_losses11)
    # model.Calculate_losses2 = Constraint(model.links,rule=Calculate_losses2)
    # model.Calculate_losses22 = Constraint(model.links,rule=Calculate_losses22)
    # model.Calculate_losses3 = Constraint(model.links,rule=Calculate_losses3)
    # model.Calculate_losses33 = Constraint(model.links,rule=Calculate_losses33)
    # model.bound_losses = Constraint(model.links,rule=bound_losses)
    # model.final_losses = Constraint(model.links,rule=final_losses)
    def ObjectiveFunction(model):
        return summation(model.weights, model.x) * model.cf / 1000 + summation(model.ps_cost, model.k) #+ summation(model.FinalLosses,model.x)*24*365*0.1*40*50
        # return summation(model.cost_asset,model.flex_positive) + return summation(model.cost_asset, model.flex_negative)
...
def MILP_base_losses2(gisele_folder,case_study,n_clusters,coe,voltage,resistance,reactance,Pmax,line_cost):
    ...
    model.Balance_rule = Constraint(rule = Balance_rule)
    def anti_paralel(model, i, j):
        return model.x[i, j] + model.x[j, i] <= 1
    ...
    ####################Define objective function##########################
    option = 'Detailed' #'LessDetailed'
    model.coef_lambda0 = Var(model.links, within=NonNegativeReals, bounds=(0, 1))
    model.coef_lambda1 = Var(model.links, within=NonNegativeReals, bounds=(0, 1))
    model.coef_lambda2 = Var(model.links, within=NonNegativeReals, bounds=(0, 1))
    model.coef_lambda3 = Var(model.links, within=NonNegativeReals, bounds=(0, 1))
    if option == 'Detailed':

        model.lambda0_bool = Var(model.links, within=Binary)
        model.lambda1_bool = Var(model.links, within=Binary)
        model.lambda2_bool = Var(model.links, within=Binary)


        def Losses1(model, i, j):
            return model.coef_lambda0[i,j]*0 + model.coef_lambda1[i,j] + model.coef_lambda2[i,j]*2 +model.coef_lambda3[i,j]*3 \
                   == model.P[i,j] # +model.coef_lambda0[j,i]*0 + model.coef_lambda1[j,i] + model.coef_lambda2[j,i]*2 +model.coef_lambda3[j,i]*3

        def Losses2(model, i , j):
            return model.coef_lambda0[i,j] + model.coef_lambda1[i,j] + model.coef_lambda2[i,j] + model.coef_lambda3[i,j]\
                    == 1 #+model.coef_lambda0[j,i] + model.coef_lambda1[j,i] + model.coef_lambda2[j,i] + model.coef_lambda3[j,i]

        def Losses3(model, i , j):
            return model.coef_lambda0[i,j]<=model.lambda0_bool[i,j]

        def Losses4(model, i , j):
            return model.coef_lambda1[i,j]<=model.lambda0_bool[i,j] +model.lambda1_bool[i,j]
        def Losses5(model, i , j):
            return model.coef_lambda2[i,j]<=model.lambda1_bool[i,j] +model.lambda2_bool[i,j]

        def Losses6(model, i , j):
            return model.coef_lambda3[i,j]<=model.lambda2_bool[i,j]
        def Losses7(model,i,j):
            return model.lambda0_bool[i,j] + model.lambda1_bool[i,j] + model.lambda2_bool[i,j] == 1

        model.Losses1 = Constraint(model.links, rule=Losses1)
        model.Losses2 = Constraint(model.links, rule=Losses2)
        model.Losses3 = Constraint(model.links, rule=Losses3)
        model.Losses4 = Constraint(model.links, rule=Losses4)
        model.Losses5 = Constraint(model.links, rule=Losses5)
        model.Losses6 = Constraint(model.links, rule=Losses6)
        model.Losses7 = Constraint(model.links, rule=Losses7)
    elif option == 'LessDetailed':
        def Losses1(model, i, j):
            return model.coef_lambda0[i,j]*0 + model.coef_lambda1[i,j] + model.coef_lambda2[i,j]*2 +model.coef_lambda3[i,j]*3 == model.P[i,j]

        def Losses2(model, i , j):
            return model.coef_lambda0[i,j] + model.coef_lambda1[i,j] + model.coef_lambda2[i,j] + model.coef_lambda3[i,j] == 1

        model.Losses1 = Constraint(model.links, rule=Losses1)
        model.Losses2 = Constraint(model.links, rule=Losses2)

    def ObjectiveFunction(model):
        return summation(model.weights, model.x) * model.cf / 1000 + summation(model.ps_cost, model.k) +24 * 365 * 0.3 * 40 * 75/1000*\
        sum((model.coef_lambda1[i,j] +model.coef_lambda2[i,j]*4+model.coef_lambda3[i,j]*9)/0.9*model.dist[i,j]*model.Z  for [i,j] in model.links)
        # Regarding the losses formulation -> 24 hours, 365 days, 0.3 is a factor to consider that not all hours have peak load ( to be calculated), 40 no idea, 75 e/MWh price of energy,
        # and divided by 1000 because the distances of the lines are in meters.
    model.Obj = Objective(rule=ObjectiveFunction, sense=minimize)
    ...
    lambda0 = instance.coef_lambda0
    lambda1 = instance.coef_lambda1
    lambda2 = instance.coef_lambda2
    lambda3 = instance.coef_lambda3
    bool1 = instance.lambda0_bool
    bool2 = instance.lambda1_bool
    bool3 = instance.lambda2_bool
    links_clusters = instance.links_clusters
    # voltage_drop=instance.z
    connections_output = pd.DataFrame(columns=[['id1', 'id2', 'power']])
    PrSubstation = pd.DataFrame(columns=[['index', 'power']])
    ...
    for index in links:
        if int(round(value(links[index]))) == 1:
            ...
            connections_output.loc[k, 'id2'] = index[1]
            connections_output.loc[k, 'power'] = value(power[index])
            connections_output.loc[k, 'lambda0'] = value(lambda0[index])
            connections_output.loc[k, 'lambda1'] = value(lambda1[index])
            connections_output.loc[k, 'lambda2'] = value(lambda2[index])
            connections_output.loc[k, 'lambda3'] = value(lambda3[index])
            connections_output.loc[k, 'bool1'] = value(bool1[index])
            connections_output.loc[k, 'bool2'] = value(bool2[index])
            connections_output.loc[k, 'bool3'] = value(bool3[index])
            k = k + 1
    k = 0
    for index in subs:
        if int(round(value(subs[index]))) == 1:
    ...
    for index in power:
        ...
        all_lines.loc[k, 'id2'] = index[1]
        all_lines.loc[k, 'power'] = value(power[index])
        all_lines.loc[k, 'lambda0'] = value(lambda0[index])
        all_lines.loc[k, 'lambda1'] = value(lambda1[index])
        all_lines.loc[k, 'lambda2'] = value(lambda2[index])
        all_lines.loc[k, 'lambda3'] = value(lambda3[index])
        all_lines.loc[k, 'bool1'] = value(bool1[index])
        all_lines.loc[k, 'bool2'] = value(bool2[index])
        all_lines.loc[k, 'bool3'] = value(bool3[index])
        k = k + 1
    k = 0
    ...
    for index in links_clusters:
        ...
        Links_Clusters.loc[k, 'power'] = value(power[index])
        Links_Clusters.loc[k, 'power'] = value(power[index])
        Links_Clusters.loc[k, 'lambda0'] = value(lambda0[index])
        Links_Clusters.loc[k, 'lambda1'] = value(lambda1[index])
        Links_Clusters.loc[k, 'lambda2'] = value(lambda2[index])
        Links_Clusters.loc[k, 'lambda3'] = value(lambda3[index])
        Links_Clusters.loc[k, 'bool1'] = value(bool1[index])
        Links_Clusters.loc[k, 'bool2'] = value(bool2[index])
        Links_Clusters.loc[k, 'bool3'] = value(bool3[index])
        k = k + 1
