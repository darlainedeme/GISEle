...
def download_osm_data(polygon, tags, file_path):
    try:
        ...
        if tag_key == 'power' and tags[tag_key] in ['substation', 'transformer']:
            st.write(f"{len(data)} substations identified")
        return file_path
    except Exception as e:
        st.error(f"Error downloading OSM data: {e}")
        return None

def download_substations_data(polygon):
    substations_file = os.path.join('data', '2_downloaded_input_data', 'substations', 'osm_substations.geojson')
    os.makedirs(os.path.dirname(substations_file), exist_ok=True)
    return download_osm_data(polygon, {'power': ['substation', 'transformer']}, substations_file)
