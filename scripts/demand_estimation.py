...
initial_values = {
    "High-Income Household": {
        "num_users": 25,
        "appliances": [
            {"name": "Indoor Bulb", "number": 6, "power": 7, "num_windows": 2, "func_time": 120, "time_fraction_random_variability": 0.1, "func_cycle": 5, "fixed": "no", "fixed_cycle": 0, "occasional_use": 1, "flat": "no", "thermal_P_var": 0, "pref_index": 0, "wd_we_type": 2, "windows": [(1170, 1440), (0, 30)]},
            {"name": "Television", "number": 2, "power": 60, "num_windows": 3, "func_time": 180, "time_fraction_random_variability": 0.1, "func_cycle": 5, "fixed": "no", "fixed_cycle": 0, "occasional_use": 1, "flat": "no", "thermal_P_var": 0, "pref_index": 0, "wd_we_type": 2, "windows": [(720, 900), (1170, 1440), (0, 60)]},
...
def display_user_category(category_name, category_data):
    ...
    for appliance in appliances:
        with st.expander(f"{appliance['name']}"):
            ...
            appliance["occasional_use"] = st.number_input(f"Occasional Use for {appliance['name']}", min_value=0.0, max_value=1.0, value=appliance["occasional_use"])
            appliance["flat"] = st.selectbox(f"Flat for {appliance['name']}", options=["yes", "no"], index=0 if appliance["flat"] == "no" else 1)
            appliance["thermal_P_var"] = st.number_input(f"Thermal Power Variability for {appliance['name']}", min_value=0.0, max_value=1.0, value=appliance["thermal_P_var"])
