...
def sensitivity(resolution, pop_points, geo_df, eps, pts, spans, case_study):
    ...
    path='Case studies/'+case_study +'/Intermediate files/Study area/Clusters/'
    tab_cluster.to_csv(path +'n_clusters.csv')
    tab_people.to_csv(path +'%_peop.csv')
    tab_area.to_csv(path +'%_area.csv')
    tab_people_area.to_csv(path +'people_area.csv')
...
def cluster_polygons (geo_df_clustered,case_study,resolution):
    '''
    Create convex hulls surrounding cluster points, to have the shapes
    '''
    geo_df_clustered_no_out = geo_df_clustered[geo_df_clustered['Cluster']!=-1]
    print(geo_df_clustered_no_out)
    geo_df_polygons = gpd.GeoDataFrame(index=geo_df_clustered_no_out['Cluster'].unique())
    for i in geo_df_clustered_no_out['Cluster'].unique():
        print(i)
        points =geo_df_clustered_no_out[geo_df_clustered_no_out['Cluster']==i].geometry
        #mtp =MultiPoint(list(points.convex_hull))
        polygon=points.buffer(int(resolution)*0.5,cap_style=3).unary_union
        print(i)
        # alpha = alphashape.optimizealpha(points)
        # hull = alphashape.alphashape(points, alpha)
        geo_df_polygons.loc[i,'geometry'] = polygon
        geo_df_polygons.loc[i, 'Cluster_ID'] =i
        print(geo_df_polygons)
    geo_df_polygons.to_file('Case studies/'+case_study +'/Intermediate files/Study area/Clusters/cluster_polygons.json',driver='GeoJSON')
