...
def rasters_to_points(study_area,crs,resolution_points,dir,protected_areas,streets,resolution_population):
    ...
    # Population
    # population_Raster=rasterio.open(dir+'/Intermediate/Geospatial_Data/Population_' + str(crs) + '.tif')
    ...
    # with rasterio.open(dir+'/Intermediate/Geospatial_Data/Population_resampled.tif',
    #                  'w', **profile_population) as dst:
    #     dst.write(data_population)
    # Population = rasterio.open(dir+'/Intermediate/Geospatial_Data/Population_resampled.tif')
    ...
    with rasterio.open(dir+'/Intermediate/Geospatial_Data/Elevation_resampled.tif',
    ...
    with rasterio.open(dir+'/Intermediate/Geospatial_Data/Slope_resampled.tif',
    ...
    with rasterio.open(dir+'/Intermediate/Geospatial_Data/Landcover_resampled.tif',
...
def create_input_csv(crs,resolution,resolution_population,landcover_option,country,case_study,database,study_area):
    ...
    input_raster = gdal.Open(dir+'/Intermediate/Geospatial_Data/Elevation.tif')
    ...
    with rasterio.open(dir+'/Intermediate/Geospatial_Data/Slope.tif', "w", **out_meta) as dest:
        dest.write(out_image)
    input_raster = gdal.Open(dir+'/Intermediate/Geospatial_Data/Slope.tif')
    ...
    # with rasterio.open(dir+'/Intermediate/Geospatial_Data/Population.tif', "w", **out_meta) as dest:
    #     dest.write(out_image)
    # # for now there is no need since the population is already in the desired crs
    # input_raster = gdal.Open(dir+'/Intermediate/Geospatial_Data/Population.tif')
    # output_raster = dir+'/Intermediate/Geospatial_Data/Population_' + str(crs) + '.tif'
    # warp = gdal.Warp(output_raster, input_raster, dstSRS=crs_str)
    # warp = None  # Closes the files
    ...
    with rasterio.open(dir+'/Intermediate/Geospatial_Data/LandCover.tif', "w", **out_meta) as dest:
        dest.write(out_image)
    input_raster = gdal.Open(dir+'/Intermediate/Geospatial_Data/LandCover.tif')
    ...
    '''Perform the weighing of the grid of points'''
...
#     df_weighted.to_csv(dir+'/Input/'+case_study+'_weighted.csv', index=False)
    '''Delete all the residue files from the resampling,reprojecting etc'''
