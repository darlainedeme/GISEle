...
def optimize(crs,country, resolution, load_capita, pop_per_household, road_coef,Clusters, case_study, LV_distance, ss_data,
    ...
    Population = gpd.read_file(os.path.join(pop_path, 'points.shp'))  
    # except:                    
    ...
    for index, row in Clusters.iterrows():
        ...
        if population_dataset_type == 'mit':
            ...
            grid_of_points = grid_of_points[['X','Y','geometry','cons (kWh/']]
            # pdb.set_trace()
        ...
        road_lines = gpd.read_file(dir_input+'/Roads_lines/Roads_lines.shp') # darlain change to road lines
        ...
        Population_clus.to_file(dir + '/population', index=False)
        if not len(road_points)<5 and not road_lines.empty: # normal procedure in case there are roads in the cluster, actually more
            ...
            all_points = Population_clus.copy() # in this case, there is no backbone - all_points is just the final houses
        ...
        # if row['cluster_ID'] == 18: 
        #     flagErrorCluster = True
        #     if flagErrorCluster == True :  
        #         errorIdList = [9815,9864,9871,9852,9837,9823]
        #         for errorId in errorIdList:
        #             clustered_points.loc[clustered_points['ID'] == errorId, 'Cluster'] = clustered_points.loc[clustered_points['ID'] == 9761, 'Cluster'].values[0]        
        ...
        for i in range(int(number_clusters)):
            ...
            # we might decide to simply assign a stand alone system
                ...
                try:  
                    sum_pop = subset['cons (kWh/'].sum() 
                    load = sum_pop/normalization * 0.35 #coincidence_factor(sum_pop, pop_per_household)
                except: 
                    sum_pop = subset['Population'].sum()
